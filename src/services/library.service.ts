  ...
  async browse(filters: Record<string, string>) {
    ...
    for (const res of resources) {
      ...
      if (targetGroup) {
          ...
          const tag = res.tags.find(t => t.group === targetGroup);
          if (tag) {
          ...
          // Priority 1: Check for LESSON tag
          const lessonTag = res.tags.find(t => t.group === "LESSON");
          if (lessonTag) {
              nextOptions.add(lessonTag.name);
              isInsideFolder = true; 
          } else {
              // Priority 2: If NO Lesson tag, check for RESOURCE_TYPE tag (Global item)
              const typeTag = res.tags.find(t => t.group === "RESOURCE_TYPE");
              if (typeTag) {
    ...
    if (targetGroup && STANDARD_HIERARCHY.includes(targetGroup) && finalOptions.length === 0 && finalResources.length > 0) {
        ...
        if (idx < STANDARD_HIERARCHY.length - 1) {
            ...
            for (const res of finalResources) {
                const tag = res.tags.find(t => t.group === nextStandard);
                if (tag) {
    ...
    if (showFiles && finalResources.length === 0 && resources.length > 0) {
          ...
          // Unless we had isLessonLevel=true but found nothing? 
          if(isLessonLevel && finalOptions.length === 0) {
             // We expected Lessons/Syllabus but found none. Show all as files.
             // This might happen if "Loose" logic filtered them out? No, they would be loose.
             // But let's be safe.
             finalResources = resources.map(res => ({
                id: res.id,
                title: res.title,
                driveFileId: res.driveFileId,
                mimeType: res.mimeType,
    ...
    return {
      nextLevel: finalOptions.length > 0 ? (finalGroup || "MIXED") : null, 
      folders: finalOptions, 
